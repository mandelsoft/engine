...
func (p *Processor) processElement(lctx common.Logging, cmd string, id ElementId) pool.Status {
	...
	elem := p.processingModel.GetElement(id)
	if elem == nil {
		if cmd != CMD_EXT {
...
func (p *Processor) setupNewInternalObject(log logging.Logger, ni *namespaceInfo, i model.InternalObject, phase model.Phase, runid model.RunId) Element {
	...
	tolock := p.processingModel.MetaModel().GetDependentTypePhases(common.NewTypeId(i.GetType(), phase))
...
func (p *Processor) internalObjectDeleted(log logging.Logger, ni *namespaceInfo, elem Element) {
	...
	for _, ph := range p.processingModel.MetaModel().Phases(elem.GetType()) {
		for _, c := range ni.GetChildren(common.NewElementIdForPhase(elem, ph)) {
...
func (p *Processor) notifyTargetWaitingState(lctx common.Logging, log logging.Logger, e Element, missing, waiting []ElementId, inputs model.Inputs) bool {
	...
	if len(waiting) > 0 || len(missing) > 0 {
		...
		log.Info("inputs not ready", keys...)
		if len(missing) > 0 {
			p.updateStatus(lctx, log, e, common.STATUS_WAITING, fmt.Sprintf("unresolved dependencies %s", utils.Join(missing)), e.GetLock())
		} else {
			p.updateStatus(lctx, log, e, common.STATUS_PENDING, fmt.Sprintf("waiting for %s", utils.Join(waiting)), e.GetLock())
		}
		return true
	...
	if e.GetObject().GetTargetState(e.GetPhase()) == nil {
		...
		if len(exttypes) > 0 {
			...
			for _, t := range exttypes {
				...
				state := o.GetState()
				v := state.GetVersion()
				err = o.UpdateStatus(lctx, p.processingModel.ObjectBase(), e.Id(), common.StatusUpdate{
...
func (p *Processor) lockGraph(lctx common.Logging, log logging.Logger, elem Element) (*model.RunId, error) {
	...
	elems := map[ElementId]common.Element{}
...
func (p *Processor) _tryLockGraph(log logging.Logger, ni *namespaceInfo, elem common.Element, elems map[ElementId]common.Element) (bool, error) {
	if elems[elem.Id()] == nil {
		...
		for _, d := range ni.GetChildren(elem.Id()) {
			ok, err := p._tryLockGraph(log, ni, d, elems)
			if !ok || err != nil {
				return false, err
			}
		}
...
func (p *Processor) _lockGraph(log logging.Logger, ns *namespaceInfo, elems map[ElementId]common.Element, id model.RunId) (bool, error) {
	var ok bool
	var err error
	...
	for _, elem := range elems {
		...
		ok, err = elem.(Element).TryLock(p.processingModel.ObjectBase(), id)
		if !ok || err != nil {
			log.Debug("locking failed for {{nestedelem}}", "nestedelem", elem.Id(), "error", err)
			return false, err
		}
		ns.pendingElements[elem.Id()] = elem.(Element)
		p.pending.Add(1)
