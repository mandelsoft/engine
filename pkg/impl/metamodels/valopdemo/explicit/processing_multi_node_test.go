...
var _ = Describe("Processing", func() {
	...
	Context("", func() {
		...
		It("node with two operands (in order)", func() {
			...
			Expect(opCcompleted.Wait(ctxutil.TimeoutContext(env.Context(), 10*time.Second))).To(BeTrue())
		...
		It("node with two operands (wrong order)", func() {
			...
			for i := 0; i < 3; i++ {
				fmt.Printf("snyc %d\n", i+1)
				Expect(nacompleted.Wait(ctxutil.TimeoutContext(env.Context(), 10*time.Second))).To(BeTrue())
			...
			Expect(nrcompleted.Wait(ctxutil.TimeoutContext(env.Context(), 10*time.Second))).To(BeTrue())
...
func (m *ValueMon) Wait(ctx context.Context) bool {
	b := m.completed.Wait(ctxutil.TimeoutContext(ctx, 10*time.Second))
	if b {
