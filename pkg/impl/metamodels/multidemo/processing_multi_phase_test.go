...
var _ = Describe("Processing", func() {
	...
	Context("", func() {
		It("single node", func() {
			...
			n5completed := proc.CompletedFuture(mmids.NewElementId(mymetamodel.TYPE_NODE_STATE, NS, "A", mymetamodel.FINAL_PHASE))
		...
		It("node with two operands (in order)", func() {
			...
			nacompleted := proc.CompletedFuture(mmids.NewElementId(mymetamodel.TYPE_NODE_STATE, NS, "C", mymetamodel.FINAL_PHASE))
		...
		It("node with two operands (wrong order)", func() {
			...
			na := db.NewOperatorNode(NS, "C", db.OP_ADD, "A", "B")
			nacompleted := proc.CompletedFuture(mmids.NewElementId(mymetamodel.TYPE_NODE_STATE, NS, "C", mymetamodel.FINAL_PHASE), true)
			MustBeSuccessfull(odb.SetObject(na))
			runtime.Gosched()
			n5 := db.NewValueNode(NS, "A", 5)
			MustBeSuccessfull(odb.SetObject(n5))
